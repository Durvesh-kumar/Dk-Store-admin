...
const TopSideBar = () => {
  ...
  return (
    <div className=" sticky top-0 left-0 border-b-2 z-50 bg-gray-300 flex lg:hidden items-center justify-around w-full gap-x-10 border shadow-md font-medium px-10 py-1">
        ...
        <div className=" relative flex items-center justify-between gap-4 md:hidden">
          ...
          <div className=" text-lg space-y-3 bg-white rounded border shadow-lg absolute top-12 left-0">
          { 
           menuDown && (
            naveLink.map((item)=>(
              <Link href={item.url} key={item.name}
              onClick={()=> setMenuDown(!menuDown)}
               className={` items-start px-4 flex gap-3 ${usePath === item.url ? 'text-blue-600 outline-1': 'text-gray-950'}`}
              >
                {item.Icon}
                <p>{item.name}</p>
              </Link>
